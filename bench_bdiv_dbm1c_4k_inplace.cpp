/*
benchmark mpn_bdiv_dbm1c_4k_inplace_func() against __gmpn_bdiv_dbm1c()

result: mpn_bdiv_dbm1c_4k_inplace_func() slightly faster, 9 tacts for n=60
*/
...
void
call_subroutine(mp_ptr tgt) {
    if constexpr (CUSTOM) {
        mpn_bdiv_dbm1c_4k_inplace_func(tgt, N, GMP_NUMB_MASK / 3);
    } else {
        (void)__gmpn_bdiv_dbm1c(tgt, tgt, N, GMP_NUMB_MASK / 3, 0);
    }
}
...
do_benchmark(const char* id, uint16_t next_n) {
    ...
    for (INT k = g_volume; k--;) {
        call_subroutine<N, CUSTOM>(g_pool_0 + i);
        i = (i + 1) & mask;
    }
    ticks = __rdtsc() - ticks;
